...
namespace ConsoleApp1.encryption
{
    static class Encryptor
    {
        ...
        public static void testEncypt(dynamic packetData)
        {
            JsonSerializerSettings serializerSettings = new JsonSerializerSettings();
            serializerSettings.NullValueHandling = NullValueHandling.Ignore;
            byte[] bytes = Encryptor.Encrypt(JsonConvert.SerializeObject(packetData, serializerSettings));

            Console.WriteLine(Encryptor.Decrypt(bytes));
        }
    }
}
