        ...
        private void HandleData(dynamic data)
        {
            ...
            switch (tag)
            {
                ...
                case "chat/message":
                    ...
                    {
                        bytes = PackageWrapper.SerializeData("chat/message", new {message = (string) message},
                            targetEncryptor);
                        // Check if sending the message was successful. 
                        bytes = !Program.SendMessageToSpecificClient((string) data.data.clientId, bytes)
                            ? PackageWrapper.SerializeData("chat/message/error",
                                new {message = "ClientId is not valid!"}, encryptor)
                            : PackageWrapper.SerializeData("chat/message/success",
                                new {message = "message has been received!"}, encryptor);
                    }
                ...
                case "chat/broadcast":
                    ...
                    {
                        bytes = PackageWrapper.SerializeData("chat/broadcast/error",
                            new {message = "message is empty!"}, encryptor);
                    }
                    else // otherwise just send the message.
                    {
                        ...
                        bytes = PackageWrapper.SerializeData("chat/broadcast/success",
                            new {message = "message has been received!"}, encryptor);
                    }
                ...
                case "client/register":
                    ...
                    if (VerifyUsername(clientName))
                    {
                        ...
                        bytes = PackageWrapper.SerializeData("client/register/success",
                            new {clientId = id, clientName, message = "Login successful."}, encryptor);
                        loggedIn = true;
                    ...
                    {
                        bytes = PackageWrapper.SerializeData("client/register/error",
                            new {message = "The username could not be set on the server."}, encryptor);
                    }
                ...
                case "client/login":
                    ...
                    {
                        name = data.data.name;
                        id = Program.registeredClients.GetValueOrDefault((name, (string) data.data.password),
                            "Error: Logged in without known id!");

                        Console.WriteLine($"[SERVER] Client logged in with id: {id}");
                        bytes = PackageWrapper.SerializeData("client/login/success",
                            new {message = "Login successful."}, encryptor);
                        loggedIn = true;
                        Program.NotifyDoctor(id, name);
                    }
                    else
                    {
                        bytes = PackageWrapper.SerializeData("client/login/error",
                            new {message = "The username or password is not correct."}, encryptor);
                    }
                ...
                case "doctor/login":
                    ...
                    {
                        bytes = PackageWrapper.SerializeData("doctor/login/error",
                            new {message = "The username or password is incorrect."}, encryptor);
                    }
                ...
                case "client/update/heartRate":
                    ...
                    Program.SendMessageToSpecificClient("0000",
                        PackageWrapper.SerializeData("client/update/heartRate",
                            new {clientId = id, data.data.heartRate}, targetEncryptor));
                    break;
                ...
                case "session/resistance":
                    ...
                    if (resistance == "")
                    {
                        bytes = PackageWrapper.SerializeData("session/resistance/error",
                            new {message = "Resistance value is invalid!"}, encryptor);
                    }
                    else
                    {
                        ...
                        {
                            bytes = PackageWrapper.SerializeData("session/resistance/error",
                                new {message = "The Client Id could not be found."}, encryptor);
                        }
                        else
                        {
                            bytes = PackageWrapper.SerializeData("session/resistance/success",
                                new {message = "Resistance has been updated."}, encryptor);
                ...
                case "session/start":
                    ...
                    if (Program.ActiveSession((string) data.data.clientId, out targetClient))
                    {
                        bytes = PackageWrapper.SerializeData("session/start/error",
                            new {message = "Session is already active."}, encryptor);
                    }
                    else
                    {
                        if (!Program.SendMessageToSpecificClient((string) data.data.clientId, bytes))
                        ...
                        else
                        {
                            ...
                            bytes = PackageWrapper.SerializeData("session/start/success",
                                new {message = "Session successfully started."}, encryptor);
                        }
                    }
                ...
                case "session/stop":
                    ...
                    if (!Program.ActiveSession((string) data.data.clientId, out targetClient))
                    {
                        bytes = PackageWrapper.SerializeData("session/stop/error",
                            new {message = "Client has no active session."}, encryptor);
                    }
                    else
                    {
                        if (!Program.SendMessageToSpecificClient((string) data.data.clientId, bytes))
                        ...
                        else
                        {
                            ...
                            bytes = PackageWrapper.SerializeData("session/stop/success",
                                new {message = "Session successfully stopped."}, encryptor);
                        }
                    }
                ...
                case "doctor/clientHistory":
                    ...
                    bytes = session != null
                        ? (byte[]) PackageWrapper.SerializeData("doctor/clientHistory/success", session, encryptor)
                        : PackageWrapper.SerializeData("doctor/clientHistory/error",
                            new {message = "No session found with the given ID."}, encryptor);
                    tcpClient.GetStream().Write(bytes, 0, bytes.Length);
                    break;
